...
export function RockImagesUploaderDirect({
  rockId,
  onSuccess,
}: RockImagesUploaderDirectProps) {
    ...
    try {
      ...
      if (hasSession) {
        try {
          ...
          if (bucketListError) {
            addDebugMessage(
              `Error listing buckets: ${bucketListError.message}`
            );
          } else {
            ...
            addDebugMessage(
              `Bucket check: "${STORAGE_BUCKET}" exists: ${
                bucketExists ? "Yes" : "No"
              }`
            );
            if (!bucketExists) {
  ...
  const handleFilesChange = (newFiles: File[]) => {
    addDebugMessage(
      `Selected ${newFiles.length} files: ${newFiles
        .map((f) => f.name)
        .join(", ")}`
    );
    setFiles(newFiles);
  ...
  const uploadImage = async (file: File): Promise<string> => {
    try {
      ...
      addDebugMessage(
        `Uploading file ${file.name} (${file.size} bytes) to ${filePath}`
      );
      ...
      try {
        ...
        if (bucketListError) {
          ...
          if (!bucketExists) {
            setShowBucketHelp(true);
            throw new Error(
              `Bucket "${STORAGE_BUCKET}" not found in your Supabase project`
            );
          }
        }
      } catch (bucketError) {
      ...
      if (token) {
        addDebugMessage(
          `Using auth token for upload: ${token.substring(0, 10)}...`
        );
      ...
      try {
        ...
        if (existingFiles && existingFiles.length > 0) {
          addDebugMessage(
            `Found potentially similar file: ${existingFiles[0].name}`
          );
        }
      } catch (listError) {
      ...
      if (error) {
        ...
        if (
          ...
          addDebugMessage(
            `Permission issue detected. Trying alternative method...`
          );
          ...
          const altUploadUrl = await new Promise<string>((resolve, reject) => {
            try {
              ...
              const fullPath = `${STORAGE_BUCKET}/rocks/${Date.now()}-${Math.random()
                .toString(36)
              ...
              xhr.upload.addEventListener("progress", (event) => {
                if (event.lengthComputable) {
                  ...
                  addDebugMessage(
                    `Alternative upload progress: ${percentComplete}%`
                  );
                }
              });
              ...
              xhr.onload = () => {
                if (xhr.status >= 200 && xhr.status < 300) {
                  ...
                  reject(
                    new Error(
                      `Alternative upload failed with status ${xhr.status}`
                    )
                  );
                }
              };
              ...
              xhr.onerror = () =>
                reject(new Error("Alternative upload network error"));

              xhr.open(
                "POST",
                `${
                  import.meta.env.VITE_SUPABASE_URL
                }/storage/v1/object/${fullPath}`
              );
          ...
          if (altUploadUrl) {
            addDebugMessage(
              `Alternative upload succeeded with URL: ${altUploadUrl}`
            );
            return altUploadUrl;
          }
        }
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      addDebugMessage(
        `Successfully uploaded ${successfulUrls.length} of ${files.length} images`
      );
      ...
      if (successfulUrls.length > 0) {
        try {
          ...
          if (error) {
            ...
            const individualInserts = await Promise.all(
              imageData.map(async (image) => {
                try {
                  ...
                  if (error) {
                    addDebugMessage(
                      `Individual insert error for ${image.image_url}: ${error.message}`
                    );
                    return null;
                  }
            ...
            const successfulIndividualInserts =
              individualInserts.filter(Boolean);
            if (successfulIndividualInserts.length > 0) {
              addDebugMessage(
                `Saved ${successfulIndividualInserts.length} records individually`
              );
            }
          ...
          if (onSuccess) {
            addDebugMessage(
              "Returning URLs to parent component despite database error"
            );
            onSuccess(successfulUrls);
          }
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        ...
        <CardDescription>
          Images will be stored in the "{STORAGE_BUCKET}" bucket in Supabase
          storage
        </CardDescription>
      </CardHeader>
      <CardContent>
        ...
        {showBucketHelp && (
          <Alert variant="destructive" className="mt-4">
            ...
            <AlertDescription>
              <p className="mb-2">
                The "{STORAGE_BUCKET}" bucket needs to be created in Supabase
                Storage:
              </p>
