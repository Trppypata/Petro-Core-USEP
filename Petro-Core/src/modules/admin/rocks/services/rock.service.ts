...
export const updateRock = async (
  ...
  try {
    ...
    try {
      ...
      console.warn(
        "⚠️ Could not fetch original rock, using provided rock_code"
      );
    }
    ...
    if (originalRockCode) {
      ...
      console.log(
        "🔒 Ensuring original rock_code is preserved:",
        originalRockCode
      );
    }

    console.log(
      "🧹 Cleaned data for update:",
    ...
    throw new Error(
      error.response?.data?.message ||
        error.message ||
        "Failed to update rock. Please try again."
    );
  }
};
