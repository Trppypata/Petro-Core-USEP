...
import { Import, File, AlertTriangle, Database, FileText, Info } from 'lucide-react';
...
const MineralExcelImporter = ({ onImportComplete }: MineralExcelImporterProps) => {
  ...
  const analyzeExcelFile = async (file: File) => {
    try {
      ...
      const hasCarbonates = sheets.some(sheet => 
        sheet.toUpperCase() === 'CARBONATES' || 
        sheet.toUpperCase() === 'CARBONATE');
      
      const hasBorates = sheets.some(sheet => 
      ...
      toast.info(`File contains ${sheets.length} sheets. Carbonate: ${hasCarbonates ? 'Found' : 'Not found'}, Borate: ${hasBorates ? 'Found' : 'Not found'}`);
      
      if (!hasCarbonates || !hasBorates) {
        toast.warning('Warning: Some expected mineral categories are missing from the Excel file.');
      }
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (result.success) {
        toast.success(`Import complete! ${result.message || 'Minerals imported successfully.'}`);
      ...
      {fileAnalysis && (
        <Alert className="my-2">
          ...
          <AlertDescription>
            ...
            <p>Carbonate category: {fileAnalysis.hasCarbonates ? 'Present' : 'Missing'}</p>
            <p>Borate category: {fileAnalysis.hasBorates ? 'Present' : 'Missing'}</p>
          </AlertDescription>
        </Alert>
      )}
